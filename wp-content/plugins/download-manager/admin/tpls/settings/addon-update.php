<div class="panel panel-default">
    <div class="panel-heading">
        <div class="pull-right" style="margin-top: -3px"><a class="btn btn-xs btn-default" href="edit.php?post_type=wpdmpro&page=settings&tab=plugin-update&newpurchase=1"><i class="color-green fa fa-refresh"></i> <?php _e('Check For Updates','download-manager'); ?></a>   <a class="btn btn-default btn-xs" href="edit.php?post_type=wpdmpro&page=settings&tab=plugin-update&logout=1"><i class="fa fa-sign-out text-danger"></i> <?php _e('Logout','download-manager'); ?></a></div>
        <i class="fa fa-refresh"></i> &nbsp; <?php _e('Add-on Update','download-manager') ?></div>
                                            ...
                                            <td style="width: 100px">
                                                <?php if($item->order_status == 'Completed'){  ?>
                                                    ...
                                                    <?php } else if(isset($latest[$plugin_name]) && version_compare($latest[$plugin_name], $plugin_data['Version'], '>')){ ?>
                                                        <a href="#" data-url="<?php echo $dlu; ?>" data-action="updateaddon" data-plugin="<?php echo $plugin_name; ?>" class="btn btn-xs btn-warning btn-block btn-update"><i class="fa fa-refresh"></i> <?php _e('Update','download-manager'); ?></a>
                        ...
                        foreach($freeaddons->post_extra as $addon){
                            ...
                            <tr class="<?php if(isset($latest[$plugin_name])) { echo version_compare($latest[$plugin_name], $plugin_data['Version'], '>')?'bg-warning':(!$plugin_data?'':'bg-success'); } ?>">
                                ...
                                <td style="width: 100px">
                                    ...
                                    <?php } else if(isset($latest[$plugin_name]) && version_compare($latest[$plugin_name], $plugin_data['Version'], '>')){ ?>
                                        <a href="#" data-url="https://www.wpdownloadmanager.com/?wpdmdl=<?php echo $addon->ID; ?>" data-action="updateaddon" data-plugin="<?php echo $plugin_name; ?>" class="btn btn-xs btn-warning btn-block btn-update"><i class="fa fa-refresh"></i> <?php _e('Update','download-manager'); ?></a>
                                    <?php } else echo "<span class='text-success'><i class='fa fa-check-circle'></i> ". __('Updated','download-manager')."</span>"; ?>
            ...
            <script>
                jQuery(function($){
                    $('.btn-update').on('click', function (res) {
                        ...
                        btn.html('<i class="fa fa-refresh fa-spin"></i> <?php _e('Please Wait...','download-manager'); ?>');
